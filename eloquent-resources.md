...
application's users. Laravel's resource classes allow you to expressively and
easily transform your models and model collections into JSON.
...
This allows your response to include links and other meta information that is
...
> {tip} This is a high-level overview of resources and resource collections. You
> are highly encouraged to read the other sections of this documentation to gain a
...
JSON structure. For example, here is a simple `User` resource class:
...
attributes that should be converted to JSON when sending the response. Notice
that we can access model properties directly from the `$this` variable. This is
because a resource class will automatically proxy property and method access
down to the underlying model for convenient access. Once the resource is
defined, it may be returned from a route or controller:
...
If you are returning a collection of resources or a paginated response, you may
use the `collection` method when creating the resource instance in your route or
controller:
...
Note that this does not allow any addition of meta data that may need to be
returned with the collection. If you would like to customize the resource
...
collection's class name without the trailing `Collection` string.

For example, `UserCollection` will attempt to map the given user instances into
the `User` resource. To customize this behavior, you may override
...
> {tip} If you have not read the [concept overview](#concept-overview), you are
> highly encouraged to do so before proceeding with this documentation.

In essence, resources are simple. They only need to transform a given model into
an array. So, each resource contains a `toArray` method which translates your
model's attributes into an API friendly array that can be returned to your
users:
...
Once a resource has been defined, it may be returned directly from a route or
controller:
...
them to the array returned by your `toArray` method. In this example, we will
use the `Post` resource's `collection` method to add the user's blog posts to
...
> {tip} If you would like to include relationships only when they have already
> been loaded, check out the documentation
> on [conditional relationships](#conditional-relationships).

#### Resource Collections

While resources translate a single model into an array, resource collections
translate a collection of models into an array. It is not absolutely necessary
to define a resource collection class for each one of your model types since all
resources provide a `collection` method to generate an "ad-hoc" resource
...
will be necessary to define a resource collection:
...
If you would like to disable the wrapping of the outer-most resource, you may
use the `withoutWrapping` method on the base resource class. Typically, you
should call this method from your `AppServiceProvider` or
another [service provider](providers.md) that is loaded on every request to your
application:
...
> {note} The `withoutWrapping` method only affects the outer-most response and
> will not remove `data` keys that you manually add to your own resource
> collections.
...
You may be wondering if this will cause your outer-most resource to be wrapped
...
When returning paginated collections in a resource response, Laravel will wrap
...
You may always pass a paginator instance to the `collection` method of a
...
response entirely before it is sent back to the client. The `when` method allows
you to expressively define your resources without resorting to conditional
statements when building the array.
...
the resource response entirely before it is sent to the client.

> {note} The `mergeWhen` method should not be used within arrays that mix string
> and numeric keys. Furthermore, it should not be used within arrays with numeric
...
them only when they have actually been loaded.

Ultimately, this makes it easier to avoid "N+1" query problems within your
resources. The `whenLoaded` method may be used to conditionally load a
relationship. In order to avoid unnecessarily loading relationships, this method
accepts the name of the relationship instead of the relationship itself:
...
be removed from the resource response entirely before it is sent to the client.
...
argument. The second argument should be a Closure that defines the value to be
...
response if the resource is the outer-most resource being returned. Typically,
...
resource is the outer-most resource being rendered:
...
an `Illuminate\Http\Response` instance, allowing you full control of the
response's headers:
...
outer-most resource in a response:
