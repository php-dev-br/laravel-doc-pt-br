...
In addition to registering `Closures` as authorization callbacks, you may register class methods by passing a string containing the class name and the method. When needed, the class will be resolved via the [service container](container.md):
...
You may also choose to utilize your `Gate` defined abilities from a [form request's](validation.md#form-request-validation) `authorize` method. For example:
...
First, let's generate a policy to manage authorization for our `Post` model. You may generate a policy using the `make:policy` [artisan command](artisan.md). The generated policy will be placed in the `app/Policies` directory:
...
> **Note:** All policies are resolved via the Laravel [service container](container.md), meaning you may type-hint any needed dependencies in the policy's constructor and they will be automatically injected.
