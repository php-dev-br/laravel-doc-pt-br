...
> {note} Be sure to add a [date cast](eloquent-mutators.md##date-casting) for the `trial_ends_at` attribute within your billable model's class definition.
...
Since Stripe webhooks need to bypass Laravel's [CSRF protection](csrf.md), be sure to list the URI as an exception in your application's `App\Http\Middleware\VerifyCsrfToken` middleware or list the route outside of the `web` middleware group:
