...
typically paired with Laravel's schema builder to easily build your application'
s database schema. If you have ever had to tell a teammate to manually add a
column to their local database schema, you've faced the problem that database
migrations solve.
...
The `--table` and `--create` options may also be used to indicate the name of
the table and whether the migration will be creating a new table. These options
...
If you would like to specify a custom output path for the generated migration,
you may use the `--path` option when executing the `make:migration` command. The
...
builder, [check out its documentation](#creating-tables). For example, this
...
> {note} If you are using the [Homestead virtual machine](homestead.md), you
> should run this command from within your virtual machine.
...
You may rollback a limited number of migrations by providing the `step` option
to the `rollback` command. For example, the following command will rollback the
...
You may rollback & re-migrate a limited number of migrations by providing
the `step` option to the `refresh` command. For example, the following command
will rollback & re-migrate the last five migrations:
...
your default connection, use the `connection` method:

    Schema::connection('foo')->create('users', function (Blueprint $table) {
        $table->increments('id');
    });

You may use the following commands on the schema builder to define the table's
options:
...
the table and a `Closure` that receives a `Blueprint` instance you may use to
...
The schema builder contains a variety of column types that you may specify when
...
modifiers" you may use while adding a column to a database table. For example,
...
Below is a list of all the available column modifiers. This list does not
include the [index modifiers](#creating-indexes):
...
Before modifying a column, be sure to add the `doctrine/dbal` dependency to
your `composer.json` file. The Doctrine DBAL library is used to determine the
current state of the column and create the SQL queries needed to make the
specified adjustments to the column:
...
type or modify the column's attributes. For example, you may wish to increase
the size of a string column. To see the `change` method in action, let's
...
> {note} Only the following column types can be "changed": bigInteger, binary,
...
> unsignedInteger and unsignedSmallInteger.

#### Renaming Columns

To rename a column, you may use the `renameColumn` method on the Schema builder.
Before renaming a column, be sure to add the `doctrine/dbal` dependency to
...
> {note} Renaming any column in a table that also has a column of type `enum` is
...
the `doctrine/dbal` dependency to your `composer.json` file and run
...
> {note} Dropping or modifying multiple columns within a single migration while
> using a SQLite database is not supported.
...
we can chain the `unique` method onto the column definition:
...
Each index method accepts an optional second argument to specify the name of the
index. If omitted, the name will be derived from the names of the table and
column(s).
...
storing "emojis" in the database. If you are running a version of MySQL older
than the 5.7.7 release or MariaDB older than the 10.2.2 release, you may need to
manually configure the default string length generated by migrations in order
for MySQL to create indexes for them. You may configure this by calling
the `Schema::defaultStringLength` method within your `AppServiceProvider`:
...
To rename an index, you may use the `renameIndex` method. This method accepts
the current index name as its first argument and the desired name as its second
argument:
...
automatically assigns a reasonable name to the indexes. Concatenate the table
name, the name of the indexed column, and the index type. Here are some
examples:
...
> {note} SQLite disables foreign key constraints by default. When using SQLite,
> make sure to [enable foreign key support](database.md#configuration) in your
