...
> {tip} You may use [Valet's `share` command](https://laravel.comvalet.md#sharing-sites) to help test webhooks during local development.
...
Since Paddle webhooks need to bypass Laravel's [CSRF protection](csrf.md), be sure to list the URI as an exception in your `VerifyCsrfToken` middleware or list the route outside of the `web` middleware group:
