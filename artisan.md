...
Let's take a look at an example command. Note that we are able to inject any dependencies we need into the command's constructor. The Laravel [service container](container.md) will automatically inject all dependencies type-hinted in the constructor:
