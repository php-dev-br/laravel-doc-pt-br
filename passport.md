...
You may also configure a [scheduled job](scheduling.md) in your application's `routes/console.php` file to automatically prune your tokens on a schedule:
...
> [!NOTE]
> If your application is primarily using Passport to issue personal access tokens, consider using [Laravel Sanctum](sanctum.md), Laravel's light-weight first-party library for issuing API access tokens.
...
Passport raises events when issuing access tokens and refresh tokens. You may [listen for these events](events.md) to prune or revoke other access tokens in your database:
