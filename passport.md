...
Passport's [service provider](providers.md) registers its own database migration directory, so you should migrate your database after installing the package. The Passport migrations will create the tables your application needs to store OAuth2 clients and access tokens:
...
You may also configure a [scheduled job](scheduling.md) in your application's `App\Console\Kernel` class to automatically prune your tokens on a schedule:
...
> {tip} If your application is primarily using Passport to issue personal access tokens, consider using [Laravel Sanctum](sanctum.md), Laravel's light-weight first-party library for issuing API access tokens.
...
Passport includes an [authentication guard](authentication.md#adding-custom-guards) that will validate access tokens on incoming requests. Once you have configured the `api` guard to use the `passport` driver, you only need to specify the `auth:api` middleware on any routes that should require a valid access token:
