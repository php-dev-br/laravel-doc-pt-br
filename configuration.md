...
To obtain an instance of the application, resolve the `Illuminate\Contracts\Foundation\Application` contract via the [service container](container.md). Of course, if you are within a [service provider](providers.md), the application instance is available via the `$this->app` instance variable.
