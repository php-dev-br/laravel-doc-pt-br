...
The `--message` option of the `php artisan down` command has been removed. As an alternative, consider [pre-rendering your maintenance mode views](configuration.md#maintenance-mode) with the message of your choice.
...
If you plan to use the [job batching](queues.md#job-batching) features of Laravel 8.x, your `failed_jobs` database table will need to be updated. First, a new `uuid` column should be added to your table:
