...
To start, [install a new Laravel 5 application](installation.md) into a fresh directory in your local environment. We'll discuss each piece of the migration process in further detail below.
...
For more information on environment configuration, view the [full documentation](configuration.md#environment-configuration).
...
If you have any [IoC](container.md) bindings in `start/global.php`, move them all to the `register` method of the `app/Providers/AppServiceProvider.php` file. You may need to import the `App` facade.
