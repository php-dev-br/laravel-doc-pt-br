...
> **Note:** The array driver is typically used for running [tests](testing.md) to prevent session data from persisting.
...
First, let's access the session. We can access the session instance via the HTTP request, which can be type-hinted on a controller method. Remember, controller method dependencies are injected via the Laravel [service container](container.md):
...
To add additional drivers to Laravel's session back-end, you may use the `extend` method on the `Session` [facade](facades.md). You can call the `extend` method from the `boot` method of a [service provider](providers.md):
