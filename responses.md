...
headers, you should use the global `view` helper function.
...
> {note} Symfony HttpFoundation, which manages file downloads, requires the file
> being downloaded to have an ASCII file name.
...
accepts a callback, file name, and an optional array of headers as its
arguments:
...
facade. For example, from a [service provider's](providers.md) `boot` method:
...
a `ResponseFactory` implementation or the `response` helper:
