...
> return [Eloquent collections](eloquent-collections.md) from your
> routes or controllers? They will automatically be converted to JSON. Give it a
> shot!
...
Returning a full `Response` instance allows you to customize the response's HTTP
...
You may also return [Eloquent ORM](eloquent.md) models and
collections directly from your routes and controllers. When you do, Laravel will
...
next request to repopulate the form:
...
the `Illuminate\Contracts\Routing\ResponseFactory` [contract](contracts.md)
is returned. This contract provides several helpful methods for generating
responses.
...
return a [view](views.md) as the response's content, you should
use the `view` method:
