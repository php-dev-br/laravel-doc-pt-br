...
The `handle` method may also type-hint dependencies, and they will be automatically injected by the [service container](container.md). For example:
...
For more information on interacting with queued commands, view the full [queue documentation](queues.md).
...
Command pipe classes are resolved through the [IoC container](container.md), so feel free to type-hint any dependencies you need within their constructors.
