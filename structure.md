...
The `resources` directory contains your [views](views.md) as well
...
authenticated [via tokens](sanctum.md) and will not have access
to session state.
...
the [event broadcasting](broadcasting.md) channels that your
...
The location of the `storage` directory can be modified via
the `LARAVEL_STORAGE_PATH` environment variable.
...
Artisan commands are registered and
your [scheduled tasks](scheduling.md) are defined.
...
houses [event classes](events.md). Events may be used to alert
...
the [queueable jobs](queues.md) for your application. Jobs may be
...
referred to as "commands" since they are an implementation of
...
your [events](events.md). Event listeners receive an event
instance and perform logic in response to the event being fired. For example,
...
your [classes that represent emails](mail.md) sent by your
...
your [Eloquent model classes](eloquent.md). The Eloquent ORM
included with Laravel provides a beautiful, simple ActiveRecord implementation
for working with your database. Each database table has a corresponding "Model"
...
contains all of the "
transactional" [notifications](notifications.md) that are sent by
your application, such as simple notifications about events that happen within
your application. Laravel's notification feature abstracts sending notifications
...
the [authorization policy classes](authorization.md) for your
...
The `Providers` directory contains all of
the [service providers](providers.md) for your application.
Service providers bootstrap your application by binding services in the service
...
the [validation documentation](validation.md).
