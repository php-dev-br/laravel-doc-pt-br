...
> {tip} The results of [Eloquent](eloquent.md) queries are always returned
...
the `Collection` class at run time. For example, the following code adds
a `toUpper` method to the `Collection` class:
...
Typically, you should declare collection macros in
...
For the remainder of this documentation, we'll discuss each method available on
the `Collection` class. Remember, all of these methods may be chained to
fluently manipulate the underlying array. Furthermore, almost every method
returns a new `Collection` instance, allowing you to preserve the original copy
...
system such as [Bootstrap](https://getbootstrap.com/docs/4.1/layout/grid/).
Imagine you have a collection of [Eloquent](eloquent.md) models you want to
display in a grid:
...
The `concat` method appends the given `array` or collection values onto the end
of the collection:
...
Finally, you may also pass a callback to the `contains` method to perform your
own truth test:
...
the given callback. The callback is free to modify the item and return it, thus
forming a new collection of modified items. Then, the array is flattened by a
level:
...
you to restrict the levels of nested arrays that will be flattened.
...
> {note} Unlike most other collection methods, `forget` does not return a new
...
will be returned if the specified key does not exist:
...
The `implode` method joins the items in a collection. Its arguments depend on
...
If the collection contains simple strings or numeric values, pass the "glue" as
...
The `intersectByKeys` method removes any keys from the original collection that
...
item value into the given callback. The callback is free to modify the item and
...
The callback should return an associative array containing a single key / value
...
If the given items's keys are numeric, the values will be appended to the end of
the collection:
...
The `partition` method may be combined with the `list` PHP function to separate
...
The `pipe` method passes the collection to the given callback and returns the
result:
...
You may optionally pass an integer to `random` to specify how many items you
would like to randomly retrieve. A collection of items is always returned when
...
If the Collection has fewer items than requested, the method will throw
...
The `reject` method filters the collection using the given callback. The
callback should return `true` if the item should be removed from the resulting
collection:
...
Alternatively, you may pass in your own callback to search for the first item
...
array keys, so in this example we'll use the [`values`](#method-values) method
...
which is what the collection's `sort` method calls under the hood.
...
keeps the original array keys, so in this example we'll use
...
You can also pass your own callback to determine how to sort the collection
values:
...
If the collection contains nested arrays or objects, you should pass a key to
use for determining which values to sum:
...
In addition, you may pass your own callback to determine which values of the
collection to sum:
...
You may also pass a negative integer to take the specified amount of items from
...
while not affecting the collection itself:
...
The static `times` method creates a new collection by invoking the callback a
...
This method can be useful when combined with factories to
create [Eloquent](eloquent.md) models:
...
> {note} `toArray` also converts all of the collection's nested objects to an
> array. If you want to get the raw underlying array, use the [`all`](#method-all)
> method instead.
...
returned collection keeps the original array keys, so in this example we'll use
the [`values`](#method-values) method to reset the keys to consecutively
numbered indexes:
...
to the method evaluates to `true`:
...
The `whereIn` method filters the collection by a given key / value contained
...
The `whereNotIn` method filters the collection by a given key / value not
...
are: [`average`](#method-average), [`avg`](#method-avg), [`contains`](#method-contains), [`each`](#method-each), [`every`](#method-every), [`filter`](#method-filter), [`first`](#method-first), [`flatMap`](#method-flatmap), [`groupBy`](#method-groupby), [`keyBy`](#method-keyby), [`map`](#method-map), [`max`](#method-max), [`min`](#method-min), [`partition`](#method-partition), [`reject`](#method-reject), [`some`](#method-some), [`sortBy`](#method-sortby), [`sortByDesc`](#method-sortbydesc), [`sum`](#method-sum),
